...
- **Vitest** for testing framework (preferred for Svelte/SvelteKit projects)
- **@testing-library/svelte** for Svelte components
...
describe("PaymentForm", () => {
  ...
  it("should update reactive values when input changes", async () => {
    const user = userEvent.setup();
    const { component } = render(PaymentForm);

    const amountInput = screen.getByLabelText("Amount");

    // Test reactive behavior through UI interactions
    await user.type(amountInput, "250");

    // Verify the displayed total includes calculated tax
    expect(screen.getByText("Total: £275.00")).toBeInTheDocument();
  });
});

// Example component structure with Svelte 5 runes
// PaymentForm.svelte
<script lang="ts">
  import { validatePaymentAmount } from './payment-validator';

  let amount = $state(0);
  let error = $state<string | null>(null);

  const tax = $derived(amount * 0.1);
  const total = $derived(amount + tax);
...
```typescript
// Good - testing server-side load functions and actions
import { describe, it, expect, vi } from 'vitest';
import type { RequestEvent } from '@sveltejs/kit';
import { load, actions } from './+page.server';

describe('Payment route', () => {
  it('should load user payment methods', async () => {
    const mockEvent = {
      locals: { user: { id: 'user_123' } },
      fetch: vi.fn().mockResolvedValue({
        ok: true,
        json: async () => [
          { id: 'card_123', last4: '4242' },
          { id: 'card_456', last4: '1234' },
        ],
      }),
  ...
  it('should process payment action', async () => {
    const mockFormData = new FormData();
    mockFormData.append('amount', '100');
    mockFormData.append('cardId', 'card_123');
    ...
    const mockEvent = {
      ...
      fetch: vi.fn().mockResolvedValue({
        ok: true,
        json: async () => ({ success: true, paymentId: 'pay_789' }),
      }),
    } as unknown as RequestEvent;
    ...
    expect(result).toEqual({
      type: 'redirect',
      status: 303,
      location: '/payments/pay_789/success',
    });
  });
});
```
