<script lang="ts">
	...
	import { DateValue, DateFormatter, getLocalTimeZone, today } from '@internationalized/date';
...
<svelte:head>
	...
	<meta name="description" content="Book your appointment at Picasso Hair Salon. Choose your service, date, time, and preferred stylist." />
</svelte:head>
			...
			{#each steps as step, index}
				<div class="flex items-center">
					<div class="flex flex-col items-center">
						<div class="flex h-10 w-10 items-center justify-center rounded-full {currentStep >= step.number ? 'bg-primary text-primary-foreground' : 'bg-gray-200 text-gray-500'}">
						...
						<span class="mt-2 text-xs font-medium {currentStep >= step.number ? 'text-primary' : 'text-gray-500'}">{step.title}</span>
					</div>
					{#if index < steps.length - 1}
						<Separator class="mx-4 h-px w-16 {currentStep > step.number ? 'bg-primary' : 'bg-gray-200'}" />
					{/if}
				</div>
			{/each}
		</div>
			...
			<Card.Root>
				...
				<Card.Content class="space-y-4">
					{#each data.services as service (service.id)}
						<div 
							class="cursor-pointer rounded-lg border p-4 transition-colors hover:bg-gray-50 {selectedService === service.id ? 'border-primary bg-primary/5' : 'border-gray-200'}"
							...
							<div class="flex items-start justify-between">
								<div class="flex-1">
									...
									<div class="flex items-center gap-4 text-sm">
										<span class="font-semibold text-primary">{formatPrice(service.price)}</span>
										<span class="text-gray-500">{formatDuration(service.duration)}</span>
									</div>
								</div>
								<div class="ml-4">
									<RadioGroup.Item value={service.id} checked={selectedService === service.id} />
								</div>
							</div>
						</div>
					{/each}
			...
			<Card.Root>
				...
				<Card.Content class="space-y-4">
					{#each data.stylists as stylist (stylist.id)}
						<div 
							class="cursor-pointer rounded-lg border p-4 transition-colors hover:bg-gray-50 {selectedStylist === stylist.id ? 'border-primary bg-primary/5' : 'border-gray-200'}"
						...
						>
							<div class="flex items-start justify-between">
								...
								<div class="ml-4">
									<RadioGroup.Item value={stylist.id} checked={selectedStylist === stylist.id} />
								</div>
							</div>
						</div>
					{/each}
				...
				<Card.Footer class="flex gap-2">
					<Button variant="outline" onclick={prevStep} class="flex-1">Back</Button>
					<Button onclick={nextStep} disabled={!isStep3Valid} class="flex-1">
						Review Booking
					</Button>
				</Card.Footer>
			</Card.Root>
		{:else if currentStep === 4}
			...
			<Card.Root>
				<Card.Header>
					...
					<Card.Description>Please review your appointment details before confirming.</Card.Description>
				</Card.Header>
				<Card.Content class="space-y-6">
					<!-- Booking Summary -->
							...
							<div class="rounded-lg bg-gray-50 p-4">
								...
								<div class="space-y-3">
									...
									<div class="flex justify-between">
										...
										<span class="font-medium">{formatDuration(selectedServiceDetails.duration)}</span>
									</div>
									...
									<div class="flex justify-between">
										...
										<span class="text-xl font-bold text-primary">{formatPrice(selectedServiceDetails.price)}</span>
									</div>
								</div>
							</div>
						</div>
