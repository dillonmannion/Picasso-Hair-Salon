...
describe('RLS Policy Optimizer', () => {
  ...
  it('should add composite indexes for common query patterns', () => {
    ...
    expect(result.migration).toContain(
      'ON appointments USING btree (salon_id, appointment_date, status)'
    );
  ...
  it('should create security definer functions for complex role checks', () => {
    ...
    expect(result.migration).toContain(
      'CREATE OR REPLACE FUNCTION has_salon_access(salon_id UUID)'
    );
  ...
  it('should generate indexes for columns used in RLS policies', () => {
    ...
    expect(result.migration).toContain(
      'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_customer_bookings_customer_id'
    );
    expect(result.migration).toContain('ON customer_bookings USING btree (customer_id)');
    expect(result.migration).toContain(
      'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_customer_bookings_salon_id'
    );
    expect(result.migration).toContain(
      'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_customer_bookings_created_by'
    );
  });
  ...
  it('should validate input configuration', () => {
    ...
    expect(() => generateRLSOptimizationMigration(invalidConfig)).toThrow('Table name is required');
  });
