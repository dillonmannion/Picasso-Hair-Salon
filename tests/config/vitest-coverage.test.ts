  ...
  it('should fail the test run when coverage is below threshold', () => {
    const validateCoverage = (
      ...
      Object.entries(thresholds).forEach(([metric, threshold]) => {
        ...
        if (actual < threshold) {
          failures.push(`ERROR: ${metric} coverage of ${actual}% does not meet threshold of ${threshold}%`);
        }
      });
    ...
    expect(result.failures).toContain('ERROR: branches coverage of 75% does not meet threshold of 80%');
    expect(result.failures).toContain('ERROR: functions coverage of 78% does not meet threshold of 80%');
    expect(result.failures).toContain('ERROR: lines coverage of 79% does not meet threshold of 80%');
    expect(result.failures).toContain('ERROR: statements coverage of 77% does not meet threshold of 80%');
  });
