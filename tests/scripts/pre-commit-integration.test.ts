...
describe('Pre-commit Hook Integration', () => {
  ...
  it('should detect TypeScript files with "any" type', async () => {
    ...
    // Mock git to return this file as staged
    const mockExec = (cmd: string): Promise<{ stdout: string; stderr: string }> => {
      return new Promise((resolve) => {
        if (cmd.includes('git diff --cached')) {
          resolve({ stdout: 'src/bad.ts', stderr: '' });
        } else if (cmd.includes('eslint') && cmd.includes('bad.ts')) {
          resolve({ 
            stdout: '', 
            stderr: 'Use of "any" type is forbidden' 
          });
        } else {
  ...
  it('should check for type assertions', async () => {
    ...
    const content = await fs.readFile(filePath, 'utf-8');
    const hasTypeAssertion = content.includes(' as ') || content.includes('<') && content.includes('>');
