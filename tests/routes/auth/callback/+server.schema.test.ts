...
import { createMockOAuthCallback, createMockOAuthError, createMockProfile } from '$lib/test-utils/factories';
...
describe('OAuth callback with schema validation', () => {
  it('should validate and parse callback parameters with schemas', async () => {
    ...
    const url = new URL(`http://localhost/auth/callback?code=${mockParams.code}&next=${mockParams.next}`);
  ...
  it('should handle missing code with schema validation error', async () => {
    ...
    const mockEvent = { url, locals: { supabase: {} } };
  ...
  it('should handle OAuth error parameters with schema', async () => {
    ...
    const url = new URL(`http://localhost/auth/callback?error=${mockError.error}&error_description=${encodeURIComponent(mockError.error_description!)}`);
    const mockEvent = { url, locals: { supabase: {} } };
  ...
  it('should validate next parameter for security', async () => {
    ...
    const url = new URL(`http://localhost/auth/callback?code=${mockParams.code}&next=${encodeURIComponent(mockParams.next)}`);
    const mockEvent = { url, locals: mockLocals };
